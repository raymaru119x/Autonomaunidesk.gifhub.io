<%@ page import="controladores.UserDAO" %> 
    ...
    <body>
        ...
        <section>
            <center>
                ...
                <%
                    // Validar si se hizo una solicitud POST
                    if ("POST".equalsIgnoreCase(request.getMethod())) {
                        ...
                        try {
                            boolean success = userDAO.registerUser(usuario, password);
                            if (success) {
                                // Usuario registrado exitosamente
                                session.setAttribute("usuario", usuario); // Guarda el usuario autenticado en la sesión
                                out.println("<script>");
                                out.println("Swal.fire({");
                                out.println("    icon: 'success',");
                                out.println("    title: 'Registro exitoso',");
                                out.println("    text: 'Usuario registrado correctamente. Redirigiendo...',");
                                out.println("    timer: 3000,");
                                out.println("    showConfirmButton: false");
                                out.println("}).then(() => {");
                                out.println("    window.location.href = 'socket1.jsp';");  // Redirige a otra página
                                out.println("});");
                                out.println("</script>");
                            } else {
                                ...
                                out.println("    text: 'No se pudo registrar el usuario. Por favor, inténtalo de nuevo.',");
                                out.println("    confirmButtonText: 'Aceptar'");
                                out.println("});");
                                out.println("</script>");
                            }
                        } catch (SQLException e) {
                            // Mostrar mensaje de error específico en una alerta
                            out.println("<script>");
                            out.println("Swal.fire({");
                            out.println("    icon: 'error',");
                            out.println("    title: 'Error de servidor',");
                            out.println("    text: 'Hubo un problema al registrar el usuario: " + e.getMessage() + "',");
                            out.println("    confirmButtonText: 'Aceptar'");
                            out.println("});");
                            out.println("</script>");
                        }
